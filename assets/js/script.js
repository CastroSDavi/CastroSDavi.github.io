...
class QuizData {
    constructor(
        ...
        this.urlPerguntas = urlPerguntas; this.urlCategorias = urlCategorias; this.urlOpcoes = urlOpcoes;
        this.perguntas = []; this.categorias = []; this.opcoesResposta = [];
    }
    ...
    async _fetchJson(url, required = true) {
        try {
            ...
            if (!response.ok) {
                if (required) throw new Error(`Falha ao carregar ${url}: ${response.statusText}`);
                console.warn(`Aviso: Falha ao carregar ${url}.`); return [];
            }
            const data = await response.json();
            if (!Array.isArray(data) && required && data !== null) {
                 throw new Error(`Formato de dados inválido em ${url}.`);
            }
            return data || [];
        } catch (error) {
            if (required) throw error;
            console.warn(`Erro ao processar ${url}: ${error.message}.`); return [];
        }
    }
    ...
    async loadAllData() {
        try {
            ...
            if (!this.perguntas.length) console.warn("Nenhuma pergunta carregada.");
    ...
    _validateDataIntegrity() { /* Validações básicas de dados aqui, omitidas por brevidade na resposta, mas mantenha no seu código real */ }
    ...
    getCategoriasHierarquicamente() {
        ...
        const raizes = [];
        catMap.forEach(node => { (node.id_categoria_pai === null || !catMap.has(node.id_categoria_pai)) ? raizes.push(node) : catMap.get(node.id_categoria_pai).subcategorias.push(node); });
...
class QuizState {
    constructor() {
        this.allQuestions = []; this.filteredQuestions = []; this.currentQuestionIndex = 0;
        this.selectedCategories = []; this.selectedDifficulties = ['all'];
        this.isInitialQuestionLoad = true; this.isQuickQuizMode = false; this.QUICK_QUIZ_COUNT = 10;
    }
...
class QuizUI {
    ...
    cacheDOMelements() {
        this.elements = { // Mapeamento de elementos da UI
            ...
            // Adicionar o novo botão da home se for referenciado diretamente (geralmente não precisa se usar data-section)
            // goToChallengesHubBtn: document.getElementById('go-to-challenges-hub')
        };
    ...
    _validateCache() { /* Validações básicas dos elementos DOM aqui, omitidas por brevidade */ }
    ...
    toggleFilterPanel(show) {
        const { filterPanel: panel, filterPanelOverlay: overlay } = this.elements; if (!panel || !overlay) return;
    ...
    getSectionUIConfig() {
        return {
            ...
            'question-section': { visible: [this.elements.challengeHubContainer], hidden: [this.elements.scorePanel, this.elements.quizSectionContent, this.elements.resultadoCard, this.elements.btnEncerrarSessao, this.elements.questionGridContainer, this.elements.progressContainer, this.elements.progressText, this.elements.avisoContainer, this.elements.placeholderFiltrosContainer, this.elements.btnToggleExplanation],
                onEnter: () => { if (this.elements.quizSectionContent?.classList.contains(this.hiddenClassName) && this.elements.resultadoCard?.classList.contains(this.hiddenClassName)) { this.showElement(this.elements.challengeHubContainer); this.hideElement(this.elements.scorePanel); this.hideElement(this.elements.placeholderFiltrosContainer); this.hideElement(this.elements.avisoContainer); } }
            },
    ...
    applyAnswerFeedback(selectedOpId, opcoes) {
        ...
        const hasGenExpl = currentQ && currentQ.explicacao_resposta && currentQ.explicacao_resposta.trim() !== '';
    ...
    updateNavigationButtons(isFirst,isLast,totalQuestions){const n=this.elements.navigationButtons,p=this.elements.prevBtn,nxt=this.elements.nextBtn;if(n&&p&&nxt){if(totalQuestions<=0)this.hideElement(n);else{this.showElement(n);p.disabled=isFirst;nxt.disabled=false; nxt.textContent=isLast?"Resultado":" Avançar";}}}
    ...
    generateCategoryTree(categories){const treeCont=this.elements.categoryTreeList;if(!treeCont)return;treeCont.innerHTML='';const createNodes=(nodes,parentEl,level=0)=>{nodes.forEach(catNode=>{const li=document.createElement('li');li.className='category-tree__item';li.setAttribute('role','treeitem');li.setAttribute('aria-checked','false');const wrap=document.createElement('div');wrap.className='category-tree__label-wrapper';const inp=document.createElement('input');inp.type='checkbox';inp.id=`cat-tree-${catNode.id_categoria}`;inp.className='category-tree__input u-sr-only';inp.value=catNode.id_categoria.toString();inp.tabIndex=-1;inp.addEventListener('change',(e)=>this.handleCategoryCheckboxChange(e.target));const lbl=document.createElement('label');lbl.htmlFor=inp.id;lbl.className='category-tree__label';lbl.textContent=catNode.nome_categoria;lbl.tabIndex=0;lbl.addEventListener('keydown',(e)=>{if(e.key===' '||e.key==='Enter'){inp.checked=!inp.checked;inp.dispatchEvent(new Event('change',{bubbles:true}));e.preventDefault();}});if(catNode.subcategorias?.length>0){li.classList.add('category-tree__item--has-children');li.setAttribute('aria-expanded','false');const tglBtn=document.createElement('button');tglBtn.type='button';tglBtn.className='category-tree__toggle';tglBtn.setAttribute('aria-label',`Expandir categoria ${catNode.nome_categoria}`);tglBtn.setAttribute('aria-expanded','false');tglBtn.innerHTML=`<span class="material-symbols-outlined">chevron_right</span>`;wrap.appendChild(tglBtn);wrap.appendChild(inp);wrap.appendChild(lbl);const subMenu=document.createElement('ul');subMenu.className='category-tree__submenu';subMenu.setAttribute('role','group');createNodes(catNode.subcategorias,subMenu,level+1);li.appendChild(wrap);li.appendChild(subMenu);const tglAct=(ev)=>{ev.stopPropagation();const isExp=li.getAttribute('aria-expanded')==='true';li.setAttribute('aria-expanded',String(!isExp));tglBtn.setAttribute('aria-expanded',String(!isExp));tglBtn.setAttribute('aria-label',`${!isExp?'Recolher':'Expandir'} categoria ${catNode.nome_categoria}`);subMenu.classList.toggle('category-tree__submenu--expanded');if(!isExp)subMenu.style.maxHeight=subMenu.scrollHeight+"px";else requestAnimationFrame(()=>{subMenu.style.maxHeight='0';});tglBtn.querySelector('.material-symbols-outlined').textContent=!isExp?'expand_more':'chevron_right';};tglBtn.addEventListener('click',tglAct);tglBtn.addEventListener('keydown',(ev)=>{if(ev.key===' '||ev.key==='Enter'){tglAct(ev);ev.preventDefault();}});}else{wrap.appendChild(inp);wrap.appendChild(lbl);li.appendChild(wrap);}parentEl.appendChild(li);});};createNodes(categories,treeCont);const pLis=Array.from(this.elements.categoryTreeList?.querySelectorAll('.category-tree__item--has-children')||[]);for(let i=pLis.length-1;i>=0;i--)this.updateParentCheckboxState(pLis[i]);}
    handleCategoryCheckboxChange(checkbox){const isChk=checkbox.checked;const li=checkbox.closest('.category-tree__item');if(!li)return;checkbox.classList.remove('is-indeterminate');li.setAttribute('aria-checked',String(isChk));const chChks=li.querySelectorAll(':scope > .category-tree__submenu .category-tree__input');chChks.forEach(chCb=>{chCb.checked=isChk;chCb.classList.remove('is-indeterminate');const chLi=chCb.closest('.category-tree__item');if(chLi)chLi.setAttribute('aria-checked',String(isChk));});this.updateParentCheckboxState(li.parentElement?.closest('.category-tree__item'));}
    updateParentCheckboxState(parentLi){if(!parentLi)return;const pChk=parentLi.querySelector(':scope > .category-tree__label-wrapper > .category-tree__input');if(!pChk)return;const chLis=parentLi.querySelectorAll(':scope > .category-tree__submenu > .category-tree__item');if(chLis.length===0)return;let allCh=true,noCh=true,someIndet=false;chLis.forEach(chLi=>{const chInp=chLi.querySelector(':scope > .category-tree__label-wrapper > .category-tree__input');if(chInp){if(chInp.checked&&!chInp.classList.contains('is-indeterminate'))noCh=false;else if(chInp.classList.contains('is-indeterminate')){allCh=false;noCh=false;someIndet=true;}else allCh=false;}else allCh=false;});pChk.classList.remove('is-indeterminate');if(someIndet||(!allCh&&!noCh)){pChk.checked=false;pChk.classList.add('is-indeterminate');parentLi.setAttribute('aria-checked','mixed');}else if(allCh){pChk.checked=true;parentLi.setAttribute('aria-checked','true');}else{pChk.checked=false;parentLi.setAttribute('aria-checked','false');}this.updateParentCheckboxState(parentLi.parentElement?.closest('.category-tree__item'));}
    ...
    showResults(userData, totalQuestions) {
        ...
        if(resultadoMensagemMotivacional) {
            const p = userData.pontos, t = totalQuestions; let m = "Continue praticando para melhorar!";
            ...
            else if (p === 0 && userData.acertos === 0 && userData.erros === 0 && t === 0) m = "Nenhuma questão encontrada. Ajuste os filtros ou tente um Quiz Rápido.";
...
class App {
    ...
    disableCoreFunctionality() {
        ...
        // Esconder também o novo botão da home se o carregamento falhar
    ...
    setupEventListeners() {
        this.quizUI.elements.navElements?.forEach(navEl => {
            navEl.addEventListener('click', (e) => {
                ...
                } else if (targetSection === 'question-section' && navEl.id === 'go-to-challenges-hub') { // Se clicar no botão da home
                     this.quizUI.showSection('question-section'); // Garante que está na seção certa
                     // A lógica de mostrar o hub já está em showSection/onEnter
                } else if (targetSection === 'question-section' && navEl.id === 'view-all-questions-alt') { // Botão "Ver Banco de Questões"
        ...
        // Botão "Iniciar Desafio" da Home (já coberto pelo navElements se tiver data-section)
        // Se o botão 'go-to-challenges-hub' não for pego pelo seletor de navElements, adicione um listener específico:
        document.getElementById('go-to-challenges-hub')?.addEventListener('click', () => {
            this.quizUI.showSection('question-section');
        });
